...
impl Vars {
    ...
    pub fn new<const N: usize>(init: [(&str, &str); N]) -> Self {
        ...
        let exe_path = std::env::current_exe()
            ...
            .canonicalize().expect("Failed to canonicalize executable path")
            .to_string_lossy().into_owned();

        vars.set("MAKE", &exe_path, false).expect("Failed to set MAKE variable");
        vars.set("MAKEFLAGS", &crate::args::args_to_submake_str(), false)
            .expect("Failed to set MAKEFLAGS variable");
